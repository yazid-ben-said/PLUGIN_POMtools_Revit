...
def apply_ifc_config_to_options(config_data, ifc_options):
    ...
    try:
        # Set essential options first with proper error handling
...
def open_and_process_revit_files(file_paths, export_folder, config_file=None, use_active_view_only=False):
    ...
    # Important informational message about the batch process
    forms.alert(
...
def main():
    try:
        ...
        # Process the files
        results = open_and_process_revit_files(file_paths, export_folder, config_file, use_active_view_only)
